...
class MAE3D(nn.Module):
    ...
    def local_forward(self, x):
        ...
        sel_encoder_pos_embed = self.encoder_pos_embed_local.expand(batch_size, -1, -1).gather(dim=1,
                                                                                               index=sel_indices[:, :,
                                                                                                     None].expand(-1,
                                                                                                                  -1,
                                                                                                                  args.encoder_embed_dim))
        ...
        # Reshape patches and compute HOG features
        pred_patches = all_x[:, -msk_length:, :]
        target_patches = self.patch_norm(msk_x.detach())

        B, L, D = pred_patches.shape
        C = args.in_chans
        patch_size = self.patch_size
        assert D == C * patch_size ** 2, "Dimension mismatch"

        # Reshape to (B*L, C, patch_size, patch_size)
        pred_patches_reshaped = pred_patches.view(B * L, C, patch_size, patch_size)
        target_patches_reshaped = target_patches.view(B * L, C, patch_size, patch_size)

        # Extract HOG features using Kornia
        hog_pred = KF.hog(
            pred_patches_reshaped,
            orientations=9,
            pixels_per_cell=(8, 8),
            cells_per_block=(1, 1)
        )
        hog_target = KF.hog(
            target_patches_reshaped,
            orientations=9,
            pixels_per_cell=(8, 8),
            cells_per_block=(1, 1)
        )

        # Reshape HOG features and compute loss
        hog_pred = hog_pred.view(B, L, -1)
    ...
    def forward(self, x, return_image=False):
        ...
        # loss
        loss = self.criterion(input=all_x[:, -msk_length:, :],
                              target=self.patch_norm(msk_x.detach())) + 0.3 * local_loss + 0.3 * nnm_loss
