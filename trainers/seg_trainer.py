...
import models
import networks
from util import SmoothedValue, concat_all_gather, LayerDecayValueAssigner

import wandb

from data.med_transforms import get_scratch_train_transforms, get_val_transforms, get_post_transforms, get_vis_transforms, get_raw_transforms
...
def make_one_hot(input, num_classes):
    ...
    sh = list(input.shape)
    sh[1] = num_classes
    result = torch.zeros(size=sh, device=input.device)
    result = result.scatter_(dim=1, index=input.long(), value=1)
...
class SegTrainer(BaseTrainer):
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(val_loader):
                ...
                val_outputs = sliding_window_inference(val_inputs, (96, 96, 96), 2, model, overlap=args.infer_overlap)
                val_outputs = torch.softmax(val_outputs, 1).cpu().numpy()
                val_outputs = np.argmax(val_outputs, axis=1).astype(np.uint8)
                val_labels = val_labels.cpu().numpy()[:, 0, :, :, :]
                img = val_inputs[0, 0, :, :, :].cpu().numpy()
                img = np.transpose(img, (2, 1, 0))
                out_img = sitk.GetImageFromArray(img)
                sitk.WriteImage(out_img, "./mgmae_result_80.12/img/" + os.path.basename(img_name))
                label = np.transpose(val_labels[0, :, :, :], (2, 1, 0))
                out_label = sitk.GetImageFromArray(label)
                sitk.WriteImage(out_label, "./mgmae_result_80.12/label/" + "label" + os.path.basename(img_name)[3:])
                pred = np.transpose(val_outputs[0, :, :, :], (2, 1, 0))
                out_pred = sitk.GetImageFromArray(pred)
                sitk.WriteImage(out_pred, "./mgmae_result_80.12/pred/" + "label" + os.path.basename(img_name)[3:])
                dice_list_sub = []
                for i in range(1, 2):
    ...
    # calculate loss
    # @staticmethod
    def train_class_batch(self, model, samples, target, criterion):
        outputs = model(samples)  # (B, C, 96, 96, 96)
