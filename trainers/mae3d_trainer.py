...
class MAE3DTrainer(BaseTrainer):
    ...
    def epoch_train(self, epoch, niters):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            # attention_maps = model.encoder.blocks[10].attn.attention_maps
            # print(attention_maps.shape)  # (8, 12, 55, 55)
            #
            # preds = attention_maps[:, 0, :, :].cpu().numpy()
            # # Visualizing the predictions and labels
            # fig, axes = plt.subplots(1, 8, figsize=(20, 4))
            #
            # for i in range(8):
            #     # Plot predictions
            #     axes[i].imshow(preds[i, :, :], cmap=None)
            #     axes[i].axis('off')
            #     axes[i].set_title(f'Pred {i + 1}')
            #
            # plt.tight_layout()
            # plt.savefig("D:/repository/pycharm/pytorch_models/MG-MAE/att_maps/{}-{}.jpg".format(epoch, i))
    ...
    def vis_reconstruction(self, niters=0):
        ...
        for batch_data in loader:
            ...
            # visualize recon
            # unpatchify_img = self.unpatchify(recon)
            # recon_numpy = unpatchify_img.squeeze().cpu().numpy()
            # plt.figure(figsize=(10, 5))
            # plt.subplot(1, 3, 1)
            # plt.imshow(recon_numpy[48, :, :], cmap='gray')  # 沿x轴切片
            # plt.title('YZ plane')
            #
            # plt.subplot(1, 3, 2)
            # plt.imshow(recon_numpy[:, 48, :], cmap='gray')  # 沿y轴切片
            # plt.title('XZ plane')
            #
            # plt.subplot(1, 3, 3)
            # plt.imshow(recon_numpy[:, :, 48], cmap='gray')  # 沿z轴切片
            # plt.title('XY plane')
            # plt.show()
            ...
            vis_grid_hw, avg_psnr, avg_ssim, psnr_per_group, ssim_per_group = patches3d_to_grid(vis_tensor, patch_size=args.patch_size, grid_size=grid_size, in_chans=args.in_chans, hidden_axis='d')
            # import pdb
            # pdb.set_trace()
